    ...
    >
      <div className="flex gap-3 items-start">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-white leading-snug line-clamp-2">{video.title}</p>
          <p className="text-[10px] text-[#555] mt-0.5">{formatDuration(video.total_duration)} · {video.total_chunks} eps</p>

          <div className="mt-1.5 flex items-center gap-2">
            <div className="flex-1 h-[2px] bg-[#222] rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${done ? 'bg-[#0070d1]' : 'bg-[#0070d1]'}`}
                style={{ width: `${video.progress}%` }}
              />
            </div>
            <span className={`text-[10px] font-medium tabular-nums ${done ? 'text-[#0070d1]' : 'text-[#444]'}`}>
              {video.progress}%
            </span>
          </div>
        </div>
...
export default function Sidebar({ selectedVideoId, onSelectVideo }) {
  ...
  return (
    <>
      <aside className="w-[280px] flex-shrink-0 flex flex-col bg-[#080808] h-screen">
        {/* Header */}
        <div className="px-4 pt-5 pb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold tracking-tight text-white">Nibblify</span>
            </div>
          </div>
